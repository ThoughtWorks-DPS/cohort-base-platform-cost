...
output "s3_bucket_key_arn" {
  description = "RKey used to encrypt S3 bucket used for storing CUR data. This may be provisioned by this module or not."
  value       = var.s3_use_existing_kms_key ? data.aws_kms_key.s3[0].arn : aws_kms_key.s3[0].arn
}
